...
import { Movimento, MovimentoItem, User, CentroCusto, Product, LancamentoFinanceiro, Fatura } from '@/lib/types';
...
export function MovimentosTab() {
  ...
  const [allMovimentos] = useKV<Movimento[]>('club-movimentos', []);
  ...
  const filteredMovimentos = useMemo(() => {
    ...
    return (movimentos || []).filter(movimento => {
      const estadoMatch = estadoFilter === 'all' || (() => {
        if (estadoFilter === 'vencido') {
          return (movimento.estado_pagamento === 'pendente' || movimento.estado_pagamento === 'vencido') && 
      ...
      const now = new Date();
      if ((movimento.estado_pagamento === 'pendente' || movimento.estado_pagamento === 'vencido') && 
          isBefore(new Date(movimento.data_vencimento), now)) {
  ...
  const handleConfirmarLiquidacao = () => {
    const movimentosParaLiquidar = selectedMovimentoId 
      ? [selectedMovimentoId] 
      : Array.from(selectedMovimentos);
    ...
    const novosLancamentos: LancamentoFinanceiro[] = movimentosParaLiquidar.map(movimentoId => {
  ...
  const handleConfirmarDelete = () => {
    ...
    setMovimentoItens(current => (current || []).filter(item => !movimentosParaApagar.includes(item.movimento_id)));
  ...
  const handleCriarMovimento = () => {
    ...
    if (editingMovimentoId) {
      ...
      const totalAbsoluto = linhasValidas.reduce((sum, l) => sum + (l.valor_unitario * l.quantidade * (1 + l.imposto_percentual / 100)), 0);
      ...
      setMovimentoItens(current => 
        (current || []).filter(item => item.movimento_id !== editingMovimentoId)
      );
  ...
  const getFaturasAssociadas = (movimentoId: string) => {
    ...
    const faturasAssociadas = faturasIds.map(faturaId => {
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.classificacao} onValueChange={(v) => setFormData({ ...formData, classificacao: v as 'receita' | 'despesa' })}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    <Label>Tipo *</Label>
                    <Select value={formData.tipo} onValueChange={(v) => setFormData({ ...formData, tipo: v as Movimento['tipo'] })}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                              ...
                              <Select 
                                ...
                                onValueChange={(v) => {
                                  if (v && v !== 'none') {
                                    ...
                                    if (fatura) {
                                      const user = (users || []).find(u => u.id === fatura.user_id);
                                      updateLinha(index, 'descricao', `${fatura.tipo} - ${user?.nome_completo || 'Cliente'} - ${format(new Date(fatura.data_emissao), 'MM/yyyy')}`);
                                  ...
                                  if (v && v !== 'none') {
                                    ...
                                    if (movimento) {
                                      const nomeDisplay = movimento.user_id 
                                        ? (users || []).find(u => u.id === movimento.user_id)?.nome_completo 
                                        : movimento.nome_manual;
                                      updateLinha(index, 'descricao', `${movimento.tipo} - ${nomeDisplay || 'Cliente'} - ${format(new Date(movimento.data_emissao), 'dd/MM/yyyy')}`);
                ...
                <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  ...
                  <span className="text-2xl font-bold text-primary">
                    {formData.classificacao === 'despesa' ? '-' : ''}€{linhas.reduce((sum, l) => sum + (l.valor_unitario * l.quantidade * (1 + l.imposto_percentual / 100)), 0).toFixed(2)}
                  </span>
                </div>
              </div>
              <DialogFooter>
                ...
                <Button onClick={handleCriarMovimento}>
                  {editingMovimentoId ? 'Guardar Alterações' : 'Criar Movimento'}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {selectedMovimentoId 
                ? 'Liquidar Movimento' 
                : `Liquidar ${selectedMovimentos.size} Movimento(s)`}
            </DialogTitle>
            <DialogDescription>
              {selectedMovimentoId 
                ? 'Confirme o pagamento do movimento indicando o número de recibo' 
                : `Confirme o pagamento de ${selectedMovimentos.size} movimento(s) com o mesmo número de recibo`}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="numero-recibo">Número do Recibo *</Label>
              <Input 
                id="numero-recibo"
              ...
              <p className="text-xs text-muted-foreground">
                {selectedMovimentoId 
                  ? 'Insira o número do recibo para confirmar o pagamento deste movimento.'
                  : 'Este número de recibo será usado para todos os movimentos selecionados.'}
              </p>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={dialogDeleteOpen} onOpenChange={setDialogDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Esta ação é irreversível. Os movimentos serão permanentemente removidos.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Tem a certeza que deseja apagar {selectedMovimentos.size} movimento(s)? Esta ação não pode ser revertida.
            </p>
          </div>
          <DialogFooter>
            <Button 
