          ...
          <div className="space-y-2">
            {volumeByAthlete.length === 0 ? (
              ...
              volumeByAthlete.map((v, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between py-2 border-b last:border-0 hover:bg-muted/30 transition-colors px-2 rounded"
                >
                  <span className="text-sm font-medium">{v.nome_completo}</span>
              ...
              reportAttendanceByGroup.map((g, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium">{g.nome ?? 'Sem escalão'}</span>
